...
11. 加锁规则（默认RR）
	...
	4. 索引等值查询时，向右查找最后一个值不满足条件时，next-key lock 退化成间隙锁
		1. *在非唯一索引上进行范围查询加锁时，主键上也会加锁好，需要注意主键的边界*
			...
			* insert (c=5，id=6) 插入失败，(c=5，id=4) 插入成功，从这个例子中可以看出，在主键上也加了gap锁，为什么加，因为在索引中相同索引值id肯定，并且是有序的，加锁是在索引上进行的，并且通过当前顺序确定加锁范围，如果新插入一条在锁范围内的数据，相当于在破坏锁边界，破坏数据一致性，所以此时也要将左边界上的向右的id加上gap锁 (c=5,id=(5,supernum])，同理右边界上也要加类似的gap(c=15,id=[负无穷,15))
